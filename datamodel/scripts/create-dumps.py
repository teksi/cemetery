...
File | Description
------------ | -------------
tce_{version}_structure.sql | Contains the structure of tables and system schema content
tce_{version}_structure_with_value_lists.sql | Contains the structure of tables, system schema data and value lists data
tce_{version}_demo_data.backup | Data-only backup of the `tce_od` schema (i.e. ordinary data) from demonstration set of data
tce_{version}_structure_and_demo_data.backup | Complete backup with structure and data of the demonstration set of data

* If you plan to **use tce for production**, it is more likely you will be using the plain SQL `tce_{version}_structure_with_value_lists.sql`.
* If you want to **give a try at tce**, you will likely restore the `tce_{version}_structure_and_demo_data.backup` backup file.
"""  # noqa
...
def create_plain_structure_only(database: str, version: str):
    ...
    # structure
    dump_s = f"tce_{version}_structure.sql"
    ...
    # dump all from tce_sys except logged_actions
    dump_i = f"tce_{version}_pum_info.sql"
...
def create_plain_value_list(database: str, version: str, structure_dump_file: str):
    ...
    dump = f"tce_{version}_structure_with_value_lists.sql"
...
def create_backup_complete(database: str, version: str):
    ...
    dump = f"tce_{version}_structure_and_demo_data.backup"
    print(f"::group::{dump}")
    print(f"Creating dump {dump}")
    dump_file = f"datamodel/artifacts/{dump}"
    _cmd(
...
def get_parser():
    ...
    _parser.add_argument("--database", "-d", help="Sets the database name", default="tce")
    return _parser
